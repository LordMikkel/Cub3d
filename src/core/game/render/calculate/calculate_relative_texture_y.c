...
/*   Updated: 2026/02/19 17:39:35 by migarrid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calculate_relative_texture_y(t_data *data, t_ray *ray, t_txtr *texture)
{
	...
	texture->step = (double)texture->img->height / ray->line_height;
	screen_center_y = (double)data->img->height / 2.0;
	theoretical_wall_top_y = screen_center_y - (ray->line_height / 2.0);
	clipped_screen_pixels = ray->draw_start - theoretical_wall_top_y;
	texture->coord[Y] = clipped_screen_pixels * texture->step;
}
